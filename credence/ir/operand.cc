...
/**
 * @brief operand::Operand tuple as a string
 */
...
/**
 * @brief Operand types to string in reverse polish notation
 */
    ...
    std::string_view separator)
{
    ...
    std::visit(
        util::overload{ [&](std::monostate) {},
            ...
            [&](Operand::LValue const& s) { oss << s.first << space; } },
        item);
    // clang-format on
    return oss.str();
}
