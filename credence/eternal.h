  ...
  public:
    ...
    constexpr bool operator<(const Element& rhs) const noexcept
    {
        return hash < rhs.hash ||
               (!(rhs.hash < hash) && base_type::operator<(rhs));
    }
};
...
class element_hash : public element<Key, Value>
{
  ...
  public:
    ...
    constexpr element_hash(const key_type& key,
...
MAPBOX_ETERNAL_CONSTEXPR auto bound(Iterator left,
                                    Iterator right,
...
class map
{
  private:
    ...
    MAPBOX_ETERNAL_CONSTEXPR map(const T (&data)[N],
                                 std::index_sequence<I...>) noexcept
        : data_{ { data[I].first, data[I].second }... }
    {
        static_assert(sizeof...(I) == N, "index_sequence has identical length");
        // Yes, this is a bubblesort. It's usually evaluated at compile-time,
        // it's fast for data that is already sorted (like static maps), it has
        // a small code size, and it's stable.
        for (auto left = data_, right = data_ + N - 1; data_ < right;
             right = left, left = data_) {
  ...
  public:
    ...
    MAPBOX_ETERNAL_CONSTEXPR const mapped_type& at(
    ...
    MAPBOX_ETERNAL_CONSTEXPR const_iterator
    ...
    MAPBOX_ETERNAL_CONSTEXPR std::pair<const_iterator, const_iterator>
    ...
    MAPBOX_ETERNAL_CONSTEXPR std::size_t count(
        const key_type& key) const noexcept
    {
...
template<typename Key, typename Value, std::size_t N>
static constexpr auto hash_map(
    const std::pair<const Key, const Value> (&items)[N]) noexcept
{
...
constexpr std::size_t hash_offset = std::conditional_t<
    sizeof(std::size_t) < 8,
...
// FNV-1a hash
constexpr static std::size_t str_hash(
    const char* str,
    const std::size_t value = hash_offset) noexcept
{
    return *str
               ? str_hash(str + 1,
                          (value ^ static_cast<std::size_t>(*str)) * hash_prime)
               : value;
}

constexpr bool str_less(const char* lhs, const char* rhs) noexcept
{
    return *lhs && *rhs && *lhs == *rhs ? str_less(lhs + 1, rhs + 1)
                                        : *lhs < *rhs;
}
...
class string
{
  ...
  public:
    ...
    MAPBOX_ETERNAL_CONSTEXPR string& operator=(const string&) noexcept =
        default;
