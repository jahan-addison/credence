...
struct Datatype
{
    explicit constexpr Datatype() = default;
...
/**
 * @brief Get the type of an Datatype::Type variant
 */
constexpr std::string get_expression_type(Datatype::Type const& value)
{
    std::string type{};
    // clang-format off
    std::visit(
        util::overload{
            ...
            [&](Datatype::Pointer const&) { type = "pointer"; },
            [&](Datatype::Symbol const&) { type = "symbol"; },
            [&](Datatype::Unary const&) { type = "unary"; },
            [&](Datatype::Relation const&) { type = "relation"; },
            [&](Datatype::Function const&) { type = "function"; },
            [&](Datatype::LValue const&) { type = "lvalue"; } },
        value);
    // clang-format on
    return type;
}
...
inline Datatype::LValue make_lvalue(std::string const& name)
{
    return std::make_pair(name, WORD_LITERAL);
}

template<typename T>
inline Datatype::LValue make_lvalue(std::string name, T value)
{
...
inline Datatype::Type_Pointer make_value_type_pointer(
    Datatype::Type type) // not constexpr until C++23
{
    return std::make_shared<Datatype::Type>(type);
}

template<typename T>
constexpr T get_literal_from_type_pointer(Datatype::Type_Pointer const& type)
{
    static_assert(std::is_constructible_v<Literal, T>,
        "Error: Type T is not a valid alternative in datatype::Datatype");
    return std::get<T>(std::get<Literal>(*type).first);
}

inline bool is_value_type_pointer_type(Datatype::Type_Pointer const& value,
    std::string_view type)
{
    return get_expression_type(*value) == type;
}

constexpr bool is_value_type(Datatype const& expression, std::string_view type)
{
